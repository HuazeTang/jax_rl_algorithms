...
from loss.ppo_loss import PPOLoss, LossConfig
...
# Loss related
def make_loss_fn(network: nn.Module, config: AlgoConfig) -> Callable:
    loss_calculator = PPOLoss(
        network=network,
        config=LossConfig(clip_eps=config.CLIP_EPS, vf_coef=config.VF_COEF, ent_coef=config.ENT_COEF)
    )

    return loss_calculator.compute_loss
...
def update_minibatch(
    ...
    _loss_fn = make_loss_fn(network=network, config=config)
...
def make_train(config: AlgoConfig) -> Callable:
    ...
    def train(rng):
        ...
        writer = SummaryWriter(logdir=f'./logs/PPO_{config.ENV_NAME}_lr{config.LR}_steps{config.NUM_STEPS}_epochs{config.UPDATE_EPOCHS}_{timestamp}')  
